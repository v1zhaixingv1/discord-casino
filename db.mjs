...
// Reset all users and house balances to defaults (chips=0, credits=100, house=0)
const resetUsersStmt = db.prepare('UPDATE users SET chips = 0, credits = 100, updated_at = CURRENT_TIMESTAMP');
const resetHouseExactStmt = db.prepare('UPDATE house SET chips = 0, updated_at = CURRENT_TIMESTAMP WHERE id = 1');

export function resetAllBalances() {
  const tx = db.transaction(() => {
    const usersBefore = db.prepare('SELECT COUNT(*) as n FROM users').get().n;
    const usersUpdated = resetUsersStmt.run().changes;
    resetHouseExactStmt.run();
    return { usersBefore, usersUpdated, house: getHouseStmt.get().chips };
  });
  return tx();
}
...
// Total chips in circulation: sum of all user chips + house chips
export function getCasinoNetworth() {
  const house = getHouseStmt.get().chips;
  const row = sumUserChipsStmt.get();
  const users = row ? (row.total || 0) : 0;
  return house + users;
}

export function getUserBalances(discordId) {
  ensureUserStmt.run(discordId);
  const row = getUserStmt.get(discordId);
  return { chips: row.chips, credits: row.credits };
}

export function getTopUsers(limit = 10) {
  const n = Math.max(1, Math.min(25, Number(limit) || 10)); // cap between 1–25
  return topUsersStmt.all(n);
}

// ===== CHIPS: existing behaviors =====
export function addToHouse(amount, reason, adminId) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction((amt) => {
    updateHouseStmt.run(amt);
    insertTxnStmt.run('HOUSE', amt, reason || 'house top-up', adminId || null, 'CHIPS');
  });
  tx(amount);
  return getHouseStmt.get().chips;
}

export function transferFromHouseToUser(discordId, amount, reason, adminId) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction(() => {
    const house = getHouseStmt.get();
    if (house.chips < amount) throw new Error('INSUFFICIENT_HOUSE');
    ensureUserStmt.run(discordId);
    updateHouseStmt.run(-amount);
    addChipsStmt.run(amount, discordId);
    insertTxnStmt.run(discordId, amount, reason || 'admin grant', adminId || null, 'CHIPS');
    insertTxnStmt.run('HOUSE', -amount, `grant to ${discordId}${reason ? ': ' + reason : ''}`, adminId || null, 'CHIPS');
  });
  tx();
  return { ...getUserBalances(discordId), house: getHouseStmt.get().chips };
}

export function removeFromHouse(amount, reason, adminId) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction((amt) => {
    const house = getHouseStmt.get();
    if (house.chips < amt) throw new Error('INSUFFICIENT_HOUSE');
    updateHouseStmt.run(-amt);
    insertTxnStmt.run('HOUSE', -amt, reason || 'house remove', adminId || null, 'CHIPS');
  });
  tx(amount);
  return getHouseStmt.get().chips;
}

export function burnFromUser(discordId, amount, reason, adminId) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction(() => {
    ensureUserStmt.run(discordId);
    const row = getUserStmt.get(discordId);
    if (row.chips < amount) throw new Error('INSUFFICIENT_USER');

    // subtract chips only (no house update)
    addChipsStmt.run(-amount, discordId);

    // ledger: user loses CHIPS; write a BURN info row for audit clarity
    insertTxnStmt.run(discordId, -amount, reason || 'admin burn chips', adminId || null, 'CHIPS');
    insertTxnStmt.run('BURN', amount, `burn chips from ${discordId}${reason ? ': ' + reason : ''}`, adminId || null, 'CHIPS');
  });
  tx();

  return getUserBalances(discordId); // { chips, credits }
}

// Admin mint Chips directly to a user (does not affect house)
export function mintChips(discordId, amount, reason, adminId) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction(() => {
    ensureUserStmt.run(discordId);
    addChipsStmt.run(amount, discordId);
    insertTxnStmt.run(discordId, amount, reason || 'admin mint chips', adminId || null, 'CHIPS');
  });
  tx();
  return getUserBalances(discordId);
}

// Move chips from User -> House (used for game buy-ins and losses)
export function takeFromUserToHouse(discordId, amount, reason, adminId) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction(() => {
    ensureUserStmt.run(discordId);
    const row = getUserStmt.get(discordId);
    ...
    // ledger
    insertTxnStmt.run(discordId, -amount, reason || 'game stake', adminId || null, 'CHIPS');
    insertTxnStmt.run('HOUSE', amount, `stake from ${discordId}${reason ? ': ' + reason : ''}`, adminId || null, 'CHIPS');
  });
  tx();
  return { ...getUserBalances(discordId), house: getHouseStmt.get().chips };
}

// ===== CREDITS: new behaviors =====
// Admin grant Credits (house is not involved)
export function grantCredits(discordId, amount, reason, adminId) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction(() => {
    ensureUserStmt.run(discordId);
    addCreditsStmt.run(amount, discordId);
    insertTxnStmt.run(discordId, amount, reason || 'admin grant credits', adminId || null, 'CREDITS');
  });
  tx();
  return getUserBalances(discordId);
}

// Admin burn Credits from user
export function burnCredits(discordId, amount, reason, adminId) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction(() => {
    ensureUserStmt.run(discordId);
    const row = getUserStmt.get(discordId);
    if (row.credits < amount) throw new Error('INSUFFICIENT_USER_CREDITS');
    addCreditsStmt.run(-amount, discordId);
    insertTxnStmt.run(discordId, -amount, reason || 'admin burn credits', adminId || null, 'CREDITS');
    insertTxnStmt.run('BURN', amount, `burn credits from ${discordId}${reason ? ': ' + reason : ''}`, adminId || null, 'CREDITS');
  });
  tx();
  return getUserBalances(discordId);
}

// ===== Game helpers you can call from commands/games =====
// When a bet is placed with Credits and LOST -> burn credits
export function gameLoseWithCredits(discordId, amount, detail) {
  if (!Number.isInteger(amount) || amount <= 0) throw new Error('Amount must be a positive integer.');
  const tx = db.transaction(() => {
    ensureUserStmt.run(discordId);
    const row = getUserStmt.get(discordId);
    if (row.credits < amount) throw new Error('INSUFFICIENT_USER_CREDITS');
    addCreditsStmt.run(-amount, discordId);
    insertTxnStmt.run(discordId, -amount, `game loss (credits)${detail ? ': ' + detail : ''}`, null, 'CREDITS');
    insertTxnStmt.run('BURN', amount, `game loss from ${discordId}${detail ? ': ' + detail : ''}`, null, 'CREDITS');
  });
  tx();
  return getUserBalances(discordId);
}

// When a bet is placed with Credits and WON -> pay winnings in Chips from HOUSE
