...
export default async function handleGiveCredits(interaction, ctx) {
  ...
  await ctx.postCashLog(interaction, [
    ...
    // `To: Kitten • Amount: **${new Intl.NumberFormat('en-US').format(amount)}** credits${reason ? ` • Reason: ${reason}` : ''}`,
  ...
  // return interaction.reply({ content: `✅ Gave **${fmt.format(amount)}** Credits to Kitten${reason ? ` (${reason})` : ''}.\n• Kitten's Credits: **${fmt.format(credits)}**`, ephemeral: true });
}
