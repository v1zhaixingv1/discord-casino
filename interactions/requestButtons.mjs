  ...
  if (action === 'done') {
    try {
      ...
      if (type === 'buyin') {
        ...
        try { const user = await interaction.client.users.fetch(targetId); await user.send(`🪙 Buy-in: You received ${ctx.chipsAmount(amount)}. Processed by Kitten (<@${interaction.user.id}>).`); } catch {}
        ...
        try { const user = await interaction.client.users.fetch(targetId); await user.send(`💸 Cash Out: ${ctx.chipsAmount(amount)} removed from your balance. Processed by Kitten (<@${interaction.user.id}>).`); } catch {}
      } else {
